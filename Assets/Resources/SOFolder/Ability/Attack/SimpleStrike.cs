...
public class SimpleStrike : Ability
{
    ...
    {
        float result = Func.Value + (EncahntCount * Func.EnchantRate);
        int str = 0;
        //힘이 있으면 힘만큼 증가
        if (Performer.GetComponentInChildren<Stat>().Powers.Exists(power => power.Variety == PowerVariety.Strength))
        {
            str = Performer.GetComponent<Stat>().GetComponentInChildren<Stat>().Powers.Find(power => power.Variety == PowerVariety.Strength).Value;
        }
        ...
        //타겟이 취약 걸려있으면
        if (Target != null && Target.GetComponentInChildren<Stat>().Powers.Exists(power => power.Variety == PowerVariety.Fragile))
        {
            result *= 1.5f;
        }
