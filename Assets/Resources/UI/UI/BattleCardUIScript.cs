...
public class BattleCardUIScript : MonoBehaviour
{
    ...
    public void ExtinctionEffect()
    {
        MainSceneController.Instance.SEManager.BattleSEPlay(BattelSEType.Extinction);
        CardEffect.Extinction();
        StartCoroutine(Extinction());
    }
    IEnumerator Extinction()
    {
        Image[] Images = { CardBackground, CardImage,NameBanner,RareFrame,CostOrb};
        TMP_Text[] Texts = {CardDescription,CardTypeText,CardName,CostText,HandNumText };
        ...
        while (Images[0].color.a > 0.05f)
        {
            ...
            {
                text.color -= new Color(0, 0, 0, 3 * Time.deltaTime);
            }
            yield return null;
        }
        Destroy(this.gameObject);
